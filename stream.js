...
const FORCE_REFRESH_MINUTES = parseInt(process.env.FORCE_REFRESH_MINUTES) || 1; 
const SWAP_DELAY_MINUTES = parseInt(process.env.SWAP_DELAY_MINUTES) || 1; 
const NO_REFRESH_DOMAINS = ['youtube.com', 'facebook.com', 'streamed.pk']; 
...
async function initializeVideo(page, startMuted, isActivePage) {
    ...
    // 🌟 THE FIX: Ensure the background script has had enough time to apply fullscreen CSS
    // before we drop the black loading screen curtain!
    ...
    while (true) {
        ...
        if (activeStatus.status === 'HEALTHY') {
            ...
            if (isRefreshing) {
                ...
                if (waitingTimeMs >= SWAP_DELAY_MINUTES * 60 * 1000) {
                    ...
                    // 🌟 STEP 1: Put the Loading Curtain on the ACTIVE page immediately so stream sees it
                    await showLoadingUI(activePage, "SYSTEM REFRESH", "Optimizing stream servers <span class='stream-blink'>...</span>");
                    ...
                    // 🌟 STEP 2: Put the Loading Curtain on the BACKUP page BEFORE bringing it to front
                    await showLoadingUI(backupPage, "SYSTEM REFRESH", "Optimizing stream servers <span class='stream-blink'>...</span>");

                    // Now bring it to front. Audience still sees "SYSTEM REFRESH"
                    ...
                    // 🌟 STEP 3: Init video. This will isolate the player, then remove the loading curtain.
        ...
        if (activeStatus.status === 'FROZEN' || activeStatus.status === 'CRITICAL_ERROR' || activeStatus.status === 'DEAD' || activeStatus.status === 'NO_DATA_FLOW') {
            ...
            if (failStrikes >= 2) {
                ...
                // 🌟 FIX FOR EMERGENCY SWAP: Put Curtain on ACTIVE page
                ...
                if (backupStatus.status === 'HEALTHY' || backupStatus.status === 'DEAD' || backupStatus.status === 'NO_DATA_FLOW') { 
                    ...
                    // 🌟 FIX FOR EMERGENCY SWAP: Put Curtain on BACKUP page before bringing to front
