...
const STREAM_KEYS = {
    ...
    's1.9': '14204411387439_14846464841135_f5lxgcqj5y',
...
if (selectedQuality === '360p') {
    RES_W = 640; RES_H = 360; BITRATE = 800;
} else if (selectedQuality === '480p') {
    RES_W = 854; RES_H = 480; BITRATE = 1500;
} else if (selectedQuality === '720p') {
    RES_W = 1280; RES_H = 720; BITRATE = 3000;
} else if (selectedQuality === '1080p') {
    RES_W = 1920; RES_H = 1080; BITRATE = 4500;
} else {
    RES_W = 1920; RES_H = 1080; BITRATE = 6000;
}
...
if (rawUrls !== '') {
    ...
    urlList = [
        {
            url: 'https://sport4u.online',
            hangTime: 8000
        },
        {
            url: 'https://dadocric.st/player.php?id=starsp3&v=m',
            hangTime: 8000
        }
    ];
}
...
function getSafeBackupIndex(activeIndex, currentIndex, list) {
    ...
    // Exactly two URLs:
    // always choose the other server.
    // This prevents the old 1 -> 1 loop.
    if (list.length === 2) {
...
let backupUrlIndex = getSafeBackupIndex(
    currentUrlIndex,
    currentUrlIndex,
    urlList
);
...
const PROXY_ENGINE =
    process.env.PROXY_ENGINE ||
    'Cloudflare WARP (Recommended)';

const ACTIVE_STREAM_KEY =
    STREAM_KEYS[SELECTED_CHANNEL] ||
    STREAM_KEYS['1'];
...
async function createFreshBackupBrowser() {
    ...
    try {
        ...
        if (
            backupBrowser &&
            backupBrowser.isConnected()
        ) {

            try {

                const pages =
                    await backupBrowser.pages();
    ...
    console.log(
        '[✅] BACKUP RECOVERY: Fresh backup Chrome created.'
    );
...
async function createFreshActiveBrowser() {
    ...
    console.log(
        '[✅] ACTIVE RECOVERY: Fresh active Chrome created.'
    );
...
async function setupNetworkAdBlocker(page) {
    ...
    try {
            ...
            request => {
                ...
                if (
                    ...
                    if (isMaliciousAd) {
                        ...
                        console.log(
                            `[🛡️] NAVIGATION SHIELD: Blocked malicious ad redirection to -> ${targetUrl.substring(0, 70)}...`
                        );
        ...
        console.log(
            '[⚠️] Request interception setup failed.'
        );
    }
}
...
async function applyPreloadFirewall(page) {
    ...
    try {
                                ...
                                () => {
                                    ...
                                    const adElements =
                                        shadowRoot.querySelectorAll(
                                            'in-page-message, [id^="note-"], [id^="missclick-"], [id^="close-"], [src*="adexchangerapid"]'
                                        );
                                    ...
                                    if (
                                        ...
                                        console.log(
                                            '[🛡️] SHIELD: Shadow DOM Ad Detected & Destroyed!'
                                        );
                    ...
                    e => {
                        ...
                        if (
                            ...
                            if (
                                ...
                                console.log(
                                    '[🛡️] RE-DIRECT SHIELD: Blocked navigation to external ad domain.'
                                );
                    ...
                    if (
                        ...
                        overlay.innerHTML = `
                            ...
                            <div class="stream-sub">
                                Connecting to secure stream engine...
                            </div>
                        `;
        ...
        console.log(
            '[🛡️] SYSTEM SHIELD: Preload firewall safe injection caught an error.'
        );
    }
}
                ...
                potentialElements.forEach(
                    el => {
                        ...
                        const matchesText =
                            ...
                            text.includes('MUTE ME') ||
                            text.includes('STREAM UNMUTE') ||
                            text.includes('AUDIO');

                        const matchesJS =
                            onClickStr.includes('unmute') ||
                            onClickStr.includes('volume') ||
                            onClickStr.includes('audio');
                        ...
                        if (
                            ...
                            if (isVisible) {
                                ...
                                console.log(
                                    `[🔊 ENGINE]: Dynamically triggered click on element with text: "${text || 'JS Action'}"`
                                );
...
async function initializeVideo(
    ...
    try {
        ...
        if (
            ...
            console.log(
                `[*] Clicking specific Server: ${SERVER_SELECTION}`
            );
            ...
            while (
                ...
                try {
                    ...
                    if (clickSuccess) {
                        ...
                        console.log(
                            '[+] Server Button clicked successfully!'
                        );
        ...
        // =========================================================================
        // PLAY / AUTOPLAY DETECTION
        // =========================================================================
        console.log(
            '[*] Checking if Video is Autoplaying or Needs a Play Button...'
        );
        ...
        while (
            ...
            for (const frame of page.frames()) {
                ...
                try {
                    ...
                    const playBtn =
                        await frame.$(
                            '.jw-icon-display[aria-label="Play"], button[data-plyr="play"], .vjs-big-play-button, [class*="unmute"], .fp-play'
                        );
        ...
        // =========================================================================
        // 🎯 SMART LIVE-VIDEO DETECTOR
        // =========================================================================
        console.log(
            '[*] Scanning ALL frames for the strongest live-video candidate...'
        );
        ...
        // -------------------------------------------------------------------------
        // Inspect ONE video in ONE frame
        // -------------------------------------------------------------------------
        const inspectVideo =
            async (
                frame,
                    ...
                    return await frame.evaluate(
                        async index => {
                            ...
                            try {
                                ...
                                if (
                                    typeof v.getVideoPlaybackQuality ===
                                    'function'
                                ) {
                                ...
                                resolve => {
                                    ...
                                    const finish =
                                        () => {
                                            ...
                                            if (
                                                callbackId !== null &&
                                                typeof v.cancelVideoFrameCallback ===
                                                'function'
                                            ) {
        ...
        // -------------------------------------------------------------------------
        // Inspect ALL frames concurrently
        // -------------------------------------------------------------------------
        const candidates = [];
        ...
        for (
            ...
            // Decoded/frame quality changed
            if (
                v.playbackFrameDelta >
                0
            ) {
                score += 18000;
            }
        ...
        // =========================================================================
        // 🔍 SECOND BEHAVIORAL VERIFICATION
        // =========================================================================
        if (
            targetFrame &&
            maxScore > 0
            ...
            try {
                ...
                const verified =
                    await targetFrame.evaluate(
                        async () => {
                            ...
                            // IMPORTANT:
                            // callback existence alone is NOT proof.
                            const realActivity =
                                timeDelta >
                                    0.05 ||
                                frameDelta >
                ...
                if (!verified.ok) {
                    ...
                    console.log(
                        '[⚠️] Candidate failed behavioral verification. Keeping page detection safe.'
                    );
                    ...
                    console.log(
                        `[✅] REAL VIDEO VERIFIED | ${verified.width}x${verified.height} | Presented Δ: ${verified.presentedDelta} | Time Δ: ${verified.timeDelta.toFixed(3)}s`
                    );
                }
        ...
        // =========================================================================
        // 🔗 MAP VERIFIED DEEP FRAME TO TOP-LEVEL IFRAME
        // =========================================================================
        if (targetFrame) {
            ...
            try {
                ...
                if (
                    ...
                    console.log(
                        '[✅] Verified stream mapped to top-level iframe.'
                    );

                } else {

                    console.log(
                        '[+] Verified stream is directly in main frame or iframe handle unavailable.'
                    );
                }
                ...
                console.log(
                    `[⚠️] Top-level iframe mapping failed safely: ${e.message}`
                );
            }
        }

        // =========================================================================
        // 🖥️ OLD WORKING PARENT IFRAME FULLSCREEN ENGINE
        // =========================================================================
        if (bestIframeHandle) {
                    ...
                    const applyParentFullscreen =
                        () => {
                            ...
                            try {
                                ...
                                // EXACT OLD WORKING FULLSCREEN METHOD
        ...
        // =========================================================================
        // 🎥 OLD WORKING INNER PLAYER FULLSCREEN ENGINE
        // =========================================================================
        if (!targetFrame) {

            // Safe fallback for the existing media-control block.
            targetFrame =
                page.mainFrame();
        }
        ...
        await targetFrame.evaluate(
            muteVideo => {
                ...
                window.isStreamMuted =
                    muteVideo;

                if (
                    window.__smartInnerPlayerInterval
                ) {
                    ...
                    () => {
                        ...
                        try {
                            ...
                            if (
                                ...
                                document
                                    .querySelectorAll(
                                        '.jw-icon-volume.jw-off, .vjs-vol-muted, .plyr__control--pressed[data-plyr="mute"]'
                                    )
                                    .forEach(
                                        btn => {
                            ...
                            // Prefer a meaningful large video.
                            for (
                                const v
                                of videos
                            ) {
                            ...
                            if (
                                ...
                                const playerWrap =
                                    realVideo.closest(
                                        '.jwplayer, #player, .plyr, .vjs-player, .shaka-video-container, [data-player]'
                                    ) ||
                                    realVideo;

                                // EXACT OLD WORKING PLAYER FULLSCREEN
                            ...
                            frame.evaluate(
                                () => {
                                    ...
                                    if (
                                        ...
                                        if (
                                            targetV.getVideoPlaybackQuality
                                        ) {
                                            ...
                                            targetV.webkitDecodedFrameCount !==
                                            undefined
                                        ) {
...
async function startWatchdog() {
    ...
    let backupWarmupTime =
        Date.now();

    const WARMUP_MAX_TIME =
        15000;

    let activeUrlStr =
        urlList[currentUrlIndex].url;

    let backupUrlStr =
        urlList[backupUrlIndex].url;

    let currentStreamStartTime =
        Date.now();

    let isRecoveryUIShown =
        false;

    while (true) {

        const activeBrowserAlive =
            activeBrowser &&
            activeBrowser.isConnected();

        const backupBrowserAlive =
            backupBrowser &&
            backupBrowser.isConnected();

        // =========================================================================
        // CASE 1: ACTIVE CHROME DEAD, BACKUP ALIVE
        // =========================================================================
        if (
            !activeBrowserAlive &&
            backupBrowserAlive
            ...
            console.log(
                '[🚨] ACTIVE CHROME DISCONNECTED'
            );

            console.log(
                '[🔄] BACKUP CHROME IS ALIVE'
            );

            console.log(
                '[⚡] PROMOTING BACKUP -> ACTIVE'
            );

            console.log(
                '[🛡️] OBS WILL CONTINUE RUNNING'
            );
            ...
            console.log(
                `[📺] NEW ACTIVE SERVER : [${currentUrlIndex}] -> ${activeUrlStr}`
            );
            ...
            console.log(
                `[🛡️] NEW BACKUP SERVER : [${backupUrlIndex}] -> ${backupUrlStr}`
            );
            ...
            try {
                ...
                console.log(
                    '[✅] NEW BACKUP SERVER IS BUFFERING IN BACKGROUND'
                );

            } catch (e) {

                console.log(
                    '[⚠️] Backup recreation failed. Watchdog will try the NEXT server.'
                );
            }
        ...
        // =========================================================================
        // CASE 2: BACKUP CHROME DEAD, ACTIVE ALIVE
        // =========================================================================
        if (
            activeBrowserAlive &&
            !backupBrowserAlive
            ...
            console.log(
                '[⚠️] BACKUP CHROME DISCONNECTED'
            );

            console.log(
                '[🛡️] ACTIVE STREAM WILL NOT BE TOUCHED'
            );

            console.log(
                '[🔄] REBUILDING BACKUP WITH NEXT SERVER'
            );
            ...
            try {
                ...
                console.log(
                    `[*] NEXT BACKUP SERVER -> [${backupUrlIndex}] ${backupUrlStr}`
                );
                ...
                console.log(
                    `[✅] BACKUP RECOVERED -> Server [${backupUrlIndex}]`
                );

            } catch (e) {

                console.log(
                    '[⚠️] Backup recovery failed. NEXT WATCHDOG CYCLE WILL TRY AGAIN.'
                );
            }
        }
        ...
        if (
            ...
            console.log(
                '[🚨] BOTH CHROME INSTANCES DISCONNECTED'
            );

            console.log(
                '[🛠️] LOCAL RECOVERY MODE'
            );

            console.log(
                '[🛑] OBS WILL NOT BE RESTARTED'
            );
            ...
            try {
                ...
                console.log(
                    `[*] RECOVERY ACTIVE -> [${currentUrlIndex}] ${activeUrlStr}`
                );

                console.log(
                    `[*] RECOVERY BACKUP -> [${backupUrlIndex}] ${backupUrlStr}`
                );
                ...
                await showLoadingUI(
                    activePage,
                    'SEARCHING SERVER',
                    "Finding a stable stream connection <span class='stream-blink'>...</span>"
                );
                ...
                try {
                    ...
                    console.log(
                        '[⚠️] Backup failed. Active stream continues.'
                    );
                }
                ...
                console.log(
                    '[✅] LOCAL RECOVERY COMPLETE — WATCHDOG CONTINUES'
                );

            } catch (e) {

                console.log(
                    `[❌] Local Chrome recovery failed: ${e.message}`
                );
        ...
        // =========================================================================
        // NORMAL WATCHDOG STATUS
        // =========================================================================
        const activeHangThresholdMs =
            urlList[
                currentUrlIndex
            ].hangTime;
        ...
        if (
            ...
            if (
                ...
                // Avoid endless self-selection even when a custom
                // URL list behaves unexpectedly.
                if (
                    backupUrlIndex ===
                        previousBackupIndex &&
                    urlList.length >
                ...
                console.log(
                    `[*] Shifting Backup Chrome to NEXT link -> Server [${backupUrlIndex}]`
                );
        ...
        if (
            ...
            const isExempted =
                NO_REFRESH_DOMAINS.some(
                    domain =>
                        activeUrlStr.includes(
                            domain
                        )
                );
        ...
        // =========================================================================
        // 🛡️ ACTIVE STREAM HANG DETECTION
        // =========================================================================
        if (
            activeStatus.status ===
                'HEALTHY'
            ...
            if (
                ...
                if (
                    ...
                    console.log(
                        '[⚠️] Stream Hang Detected! Showing Signal Recovery Shield instantly...'
                    );
                }
                ...
                if (
                    ...
                    if (
                        ...
                        console.log(
                            '[!] ⚠️ SYSTEM SHIELD: Detected Black Screen (Audio playing, but video frames stuck). Triggering HOT-SWAP.'
                        );
                    }
                ...
                if (
                    ...
                    console.log(
                        '[✅] Stream Recovered! Signal Recovery Shield removed instantly.'
                    );
                }

                for (
                    const frame
                    of activePage.frames()
                ) {
        ...
        if (
            ...
            const logBackupStatus =
                await checkPageStatus(
                    backupPage
                );

            const nextAfterBackupIndex =
                getSafeBackupIndex(
                    currentUrlIndex,
                    backupUrlIndex,
                    urlList
            ...
            const nextAfterBackupUrl =
                urlList[
                    nextAfterBackupIndex
                ].url;
            ...
            console.log(
                `[💓] ACTIVE HEARTBEAT (${activeBrowserName}): Status is ${activeStatus.status} | Video Time: ${activeStatus.currentTime ? activeStatus.currentTime.toFixed(1) + 's' : 'N/A'} (Limit: ${activeHangThresholdMs / 1000}s)`
            );

            console.log(
                `[▶️] CURRENTLY LIVE              : Server [${currentUrlIndex}] (Audio ON) -> ${activeUrlStr}`
            );
            ...
            console.log(
                `[🖤] BACKUP HEARTBEAT (${backupBrowserName}): Status is ${logBackupStatus.status} | Video Time: ${logBackupStatus.currentTime ? logBackupStatus.currentTime.toFixed(1) + 's' : 'N/A'}`
            );

            console.log(
                `[🔄] RUNNING IN BACKGROUND       : Server [${backupUrlIndex}] (Audio MUTED) -> ${backupUrlStr}`
            );

            console.log(
                `[⏭️] NEXT QUEUE (AFTER BACKUP)  : Server [${nextAfterBackupIndex}] -> ${nextAfterBackupUrl}`
            );
        ...
        if (
            ...
            if (
                ...
                console.log(
                    `[⏳] Watchdog detected '${activeStatus.status}', but stream is in WARM-UP phase. Waiting...`
                );
            ...
            if (
                ...
                console.log(
                    '[*] Initiating forward rotation to prevent stream drop...'
                );
                ...
                console.log(
                    `[💀] FAILED STREAM: Server [${currentUrlIndex}] -> ${activeUrlStr}`
                );
            ...
            console.log(
                '[*] Checking Backup Tab status before switching...'
            );
            ...
            // =========================================================================
            // SCENARIO A: BACKUP ALREADY HEALTHY
            // =========================================================================
            if (
                backupStatus.status ===
                    'HEALTHY' &&
                ...
                console.log(
                    '[⚡] BACKUP STREAM ALREADY HEALTHY'
                );

                console.log(
                    '[⚡] SHOWING TRANSITION UI & PROMOTING INSTANTLY'
                );
                ...
                await showLoadingUI(
                    backupPage,
                    'RECONNECTING',
                    "Establishing secure connection to backup server <span class='stream-blink'>...</span>"
                );
                ...
                console.log(
                    `[📺] NEW ACTIVE STREAM : Server [${currentUrlIndex}] -> ${activeUrlStr}`
                );

                console.log(
                    '[🔊] LIVE AUDIO STATUS : ON (Seamlessly Promoted & Unmuted)'
                );
                ...
                console.log(
                    `[🛡️] NEXT BACKUP QUEUE : Server [${backupUrlIndex}] -> ${backupUrlStr}`
                );
                ...
                setTimeout(
                    async () => {
                        ...
                        try {
                            ...
                            console.log(
                                '[⏳] Starting background buffer rebuilding safely...'
                            );
                            ...
                            console.log(
                                '[⚠️] Background buffer navigation handled safely.'
                            );
                        }
            ...
            // =========================================================================
                ...
                await showLoadingUI(
                    ...
                    "Optimizing current server stream <span class='stream-blink'>...</span>"
                );
                ...
                console.log(
                    '[*] Initializing Video on the newly active tab...'
                );
                ...
                console.log(
                    '[🔄] PROACTIVE REFRESH EXECUTED SUCCESSFULLY'
                );
            ...
            // =========================================================================
                ...
                console.log(
                    '[!] ❌ BOTH TABS FAILED (Active & Backup Compromised)'
                );

                console.log(
                    '[🔍] FRESH HUNTING MODE ACTIVATED: Spawning clean tabs...'
                );
                ...
                console.log(
                    `[*] Hunting Active -> Server [${currentUrlIndex}]`
                );

                console.log(
                    `[*] Hunting Backup -> Server [${backupUrlIndex}]`
                );
                ...
                try {
                    ...
                    await showLoadingUI(
                        activePage,
                        'SEARCHING SERVER',
                        "Hunting for a stable stream connection <span class='stream-blink'>...</span>"
                    );
                ...
                console.log(
                    '[*] Fresh tabs active. Waiting 15 seconds (Warm-up) for video to stabilize...'
                );
            }
        }
...
async function startDirectStreaming() {
    ...
    console.log(
        '[*] Waiting for OBS to initialize before launching browser...'
    );
    ...
    console.log(
        '[*] Attempting to connect to OBS WebSocket (Polling Engine Active)...'
    );
    ...
    for (
        ...
        try {
            ...
            console.log(
                `[⏳] OBS Port 4455 not ready yet. Retrying (${attempt}/15)...`
            );
    ...
    if (isObsConnected) {
        ...
        try {
            ...
            console.log(
                '[+] Enforced WaitingScene (Loading Bar Buffer Active)'
            );
    ...
    if (
        ...
        console.log(
            `[*] Starting browser with EXACT viewport dimensions: ${RES_W}x${RES_H} and [CLOUDFLARE WARP] Proxy...`
        );

    } else {

        console.log(
            `[*] Starting browser with EXACT viewport dimensions: ${RES_W}x${RES_H} using [DIRECT GITHUB IP]...`
        );
    }

    console.log(
        '[*] Starting ACTIVE browser instance...'
    );
    ...
    console.log(
        '[*] Starting BACKUP browser instance in background...'
    );
    ...
    console.log(
        `[*] STEP 1: Loading Server [${currentUrlIndex}] on Active Page: ${urlList[currentUrlIndex].url}`
    );
    ...
    try {
        ...
        console.log(
            '[⚠️] Network buffer safely handled for primary URL.'
        );
    }
    ...
    await showLoadingUI(
        activePage,
        'STREAM LOADING',
        "Optimizing live video connection <span class='stream-blink'>...</span>"
    );
    ...
    console.log(
        `[*] STEP 2: Silently preparing Server [${backupUrlIndex}] on Backup Page: ${urlList[backupUrlIndex].url}`
    );
    ...
    console.log(
        `[📺] CURRENT ACTIVE LIVE : Server [${currentUrlIndex}] -> ${urlList[currentUrlIndex].url}`
    );

    console.log(
        '[🔊] LIVE AUDIO STATUS   : ON (Unmuted)'
    );
    ...
    console.log(
        `[🛡️] NEXT BACKUP QUEUE   : Server [${backupUrlIndex}] -> ${urlList[backupUrlIndex].url}`
    );

    console.log(
        '[🔇] BACKUP AUDIO STATUS : MUTED (Background)'
    );
    ...
    while (true) {
        ...
        try {
            ...
            if (
                ...
                console.log(
                    '[🛡️] ACTIVE CHROME STILL ALIVE — NOT RESTARTING ENGINE.'
                );
            ...
            if (
                ...
                console.log(
                    '[🛡️] BACKUP CHROME STILL ALIVE — NOT RESTARTING ENGINE.'
                );
            ...
            console.log(
                '[⚠️] NO CHROME INSTANCE SURVIVED. Performing final recovery.'
            );
        ...
        () => {
            ...
            try {
                ...
                const targetUrls =
                    ...
                    'https://sport4u.online';

                const channel =
                    process.env.OKRU_STREAM_ID ||
                    '1';

                const quality =
                    process.env.STREAM_QUALITY ||
                    '110KBps (Balanced 480p)';

                const server =
                    process.env.SERVER_SELECTION ||
                    'None';

                const cmd =
                    `gh workflow run main.yml -f target_urls="${targetUrls}" -f okru_stream_channel="${channel}" -f stream_quality="${quality}" -f server_selection="${server}" -f proxy_engine="${PROXY_ENGINE}" -f custom_duration="None"`;
