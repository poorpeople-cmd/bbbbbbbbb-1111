                ...
                if (isReady && !isSystemSwapping) {
                    ...
                    if (activePage && !activePage.isClosed()) {
                        for (const frame of activePage.frames()) {
                            if (!frame.isDetached()) {
                                await frame.evaluate(() => { 
                                    window.__targetMuted = true; 
                                    document.querySelectorAll('video, audio').forEach(m => { try { m.muted = true; m.volume = 0.0; m.pause(); m.removeAttribute('src'); m.load(); } catch(e){} }); 
                                }).catch(()=>{});
                            }
                        }
