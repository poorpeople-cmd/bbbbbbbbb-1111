...
// iss nee same url , first time error and 2nd time correct issue ko teek kar deya fullscreen huwaa lekin bohot time lee rhaa hai 
...
// // =========================================================================================
// // 🚦 OBS GATEKEEPER: THE DOUBLE-LOCK SYSTEM (2026 FIX)
// // =========================================================================================
// async function waitForPerfectFullscreenAndPlay(page) {
//     console.log('[*] 🔒 OBS LOCKED: Verifying 80%+ Fullscreen and Active Playback...');
//     let isReady = false;
//     let attempts = 0;
    
//     while (!isReady && attempts < 40) { 
//         try {
//             // Check 1: Is the iframe actually stretched over the viewport?
//             const isFullscreen = await page.evaluate(() => {
//                 let iframes = Array.from(document.querySelectorAll('iframe'));
//                 for (let ifr of iframes) {
//                     let rect = ifr.getBoundingClientRect();
//                     // If any iframe is taking up at least 80% of screen width & height
//                     if (rect.width >= window.innerWidth * 0.8 && rect.height >= window.innerHeight * 0.8) {
//                         return true;
//                     }
//                 }
                
//                 // Backup check for direct video element
//                 let vid = document.querySelector('video');
//                 if (vid) {
//                     let vrect = vid.getBoundingClientRect();
//                     if (vrect.width >= window.innerWidth * 0.8 && vrect.height >= window.innerHeight * 0.8) {
//                         return true;
//                     }
//                 }
//                 return false;
...
//             if (isFullscreen) {
//                 // Check 2: Is the video inside that iframe actually playing?
//                 let isPlaying = false;
...
//                         return vid && vid.currentTime > 0 && !vid.paused && vid.readyState >= 3;
//                     }).catch(() => false);
...
//     if (isReady) console.log('[+] 🔓 GATEKEEPER PASSED: Fullscreen is active and stream is live!');
//     else console.log('[⚠️] GATEKEEPER TIMEOUT: Stream struggled to verify, forcing OBS unlock to prevent hang.');
...
//         // 🚀 THE BULLDOZER CSS INJECTION (Aggressively targets DaddyLive chats & iframes)
//         await page.evaluate(() => {
//             setInterval(() => {
//                 try {
...
//                     // Directly find player iframes by src (DaddyLive usually uses watch/embed)
//                     let mainIframe = iframes.find(ifr => 
//                         ifr.src && (ifr.src.includes('watch') || ifr.src.includes('embed') || ifr.src.includes('player'))
//                     );

//                     // Fallback to largest if the above fails
...
//                     // Absolutely obliterate the chat, sidebars, and ads
//                     const junkClasses = '.chat, #chat, iframe[src*="chat"], header, footer, .sidebar, .banner, .ads, [class*="overlay"]:not(#smart-stream-overlay), [id*="pop"], [class*="pop"], a[href*="extension"]';
...
// // =========================================================================================
// // 🚦 OBS GATEKEEPER: VERIFY PERFECT FULLSCREEN & PLAYBACK (2026 LATEST FIX)
// // =========================================================================================
// async function waitForPerfectFullscreenAndPlay(page) {
//     console.log('[*] 🔒 OBS LOCKED: Waiting for absolute fullscreen and active playback...');
//     let isReady = false;
//     let attempts = 0;
...
// // ⏱️ Auto-Refresh System Variables
// const FORCE_REFRESH_MINUTES = parseInt(process.env.FORCE_REFRESH_MINUTES) || 9; // 9 minutes baad naya session
// const SWAP_DELAY_MINUTES = parseInt(process.env.SWAP_DELAY_MINUTES) || 1; // 1 minute ka warmup time
// const NO_REFRESH_DOMAINS = ['youtube.com', 'facebook.com', 'streamed.pk']; // In par refresh apply nahi hoga

// function shouldAutoRefresh(url) {
//     if (!url) return false;
//     return !NO_REFRESH_DOMAINS.some(domain => url.toLowerCase().includes(domain));
// }
...
//     // 🌟 NEW: Auto-Refresh Trackers
//     let isRefreshing = false;
//     let refreshTriggeredTime = 0;
...
//             // 🌟 NEW: Auto-Refresh Logic (Trigger Phase)
//             if (!isRefreshing && shouldAutoRefresh(activeUrlStr)) {
//                 let elapsedMs = Date.now() - streamSetupTime;
//                 if (elapsedMs >= FORCE_REFRESH_MINUTES * 60 * 1000) {
//                     isRefreshing = true;
//                     refreshTriggeredTime = Date.now();
//                     console.log(`\n[♻️] AUTO-REFRESH INITIATED: Stream ran for ${FORCE_REFRESH_MINUTES} mins. Loading new session in background...`);
//                     // Load current URL in backup tab silently
//                     backupPage.goto(activeUrlStr, { waitUntil: 'domcontentloaded', timeout: 60000 }).catch(() => {});
//                 }
//             }

//             // 🌟 NEW: Auto-Refresh Logic (Execution Phase)
//             if (isRefreshing) {
//                 let waitingTimeMs = Date.now() - refreshTriggeredTime;
//                 if (waitingTimeMs >= SWAP_DELAY_MINUTES * 60 * 1000) {
//                     console.log(`\n[🔄] EXECUTING SEAMLESS SWAP: ${SWAP_DELAY_MINUTES} min warmup complete!`);

//                     // Mute the old active page first
//                     for (const frame of activePage.frames()) {
//                         try {
...
//                     // Bring new session to front
//                     await backupPage.bringToFront();
//                     try { await backupPage.mouse.click(10, 10); } catch(e){} 

//                     console.log(`[*] Isolating and unmuting the new refreshed video player...`);
//                     await initializeVideo(backupPage, false, true); 
...
//                     console.log(`[✅] AUTO-REFRESH COMPLETE. Old session moved to background.`);

//                     // Pre-load the actual backup URL in the new background tab
//                     console.log(`[*] Prepping next backup server in background: ${backupUrlStr}`);
//                     backupPage.goto(backupUrlStr, { waitUntil: 'domcontentloaded', timeout: 60000 }).catch(() => {});
//                 }
//             }
//         }

//         // Mute backup page periodically just in case
//         if (backupPage && !isRefreshing) {
//             for (const frame of backupPage.frames()) {
//                 try {
//                     if (!frame.isDetached()) {
//                         frame.evaluate(() => { 
...
//             if (isRefreshing) {
//                 let timeLeft = Math.max(0, (SWAP_DELAY_MINUTES * 60 * 1000) - (Date.now() - refreshTriggeredTime));
//                 console.log(`[♻️] BACKGROUND TASK  : Warming up fresh session. Swap in ${(timeLeft/1000).toFixed(0)}s`);
//             } else {
//                 console.log(`[⏭️] NEXT IN QUEUE    : Server [${backupUrlIndex}] (Audio MUTED) -> ${backupUrlStr}`);
//             }
...
//                     streamSetupTime = Date.now(); 
//                     isWarmupPhase = true;
//                     isRefreshing = false; // Abort proactive refresh if an emergency happened
...
// ==================== Aapne bilkul sahi point catch kiya hai ke system swap karne mein bohot late ho gaya, jabke screen pehle hi black ho chuki thi aur sirf audio chal rahi thi. Yeh issue stream link ka nahi, balki aapke Watchdog aur Page Status Checker logic ki ek limitation ka hai.

// Yahan tafseel hai ke yeh kyu hua aur code mein masla kahan par hai:

// 🔍 Asal Masla Kya Hai? (The Root Cause)
// Jab stream ke backend server se kisi wajah se video feed drop ho jati hai (lag ya buffering ki wajah se), lekin audio feed chalti rehti hai, toh web player screen ko black dikhata hai. Aapki image_dd0302.png mein exactly yahi ho raha hai: player ka UI active hai (Live badge red hai, volume icon hai), lekin canvas black hai.

// Aapka Watchdog isko fauran detect is liye nahi kar paaya kyunke woh video visual frames ko nahi dekh raha, woh sirf player ka time dekh raha hai. Jab audio play hoti hai, toh player ka time (currentTime) musalsal aage badhta rehta hai.
...
// ///// 1.2 start phase  Alhamdullah, issue yeh hai k great great great very great sab teek hai eek issue tuu nahye hai bas eek chota sa cheez hai jokey bata hu yeh logs dedoo pehely . aab baat yeh hai k jab koi like istra hutaa hai tuu stream stop hu jata hai sahey baat hai qunkey data nahey hai lekin yahaa par woo recoonnting wala animation nahey araah ahai and jaab data sahey hu jataa hai tuu time lee raha hai iskoo . 2026 latest information becarefully handle this keya aap ko pata hai 
