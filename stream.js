...
let browser = null, obsProcess = null, activePage = null, backupPage = null;
...
function setupNetworkDiagnostics(page, pageName) {
    if (!page) return;
    page.on("console", msg => {
        const type = msg.type(), text = msg.text(), textLower = text.toLowerCase();
        if (type === 'error' || type === 'warning' || textLower.includes('manifest') || textLower.includes('hls') || textLower.includes('network')) {
            console.log(`[💻] ${pageName.toUpperCase()} CONSOLE: ${text}`);
        }
    });
    page.on("requestfailed", req => {
        const url = req.url(), errText = req.failure() ? req.failure().errorText : 'UNKNOWN_ERROR';
    ...
    page.on("response", async (res) => {
        ...
        if (url.includes('.m3u8') || url.includes('.ts') || url.includes('.mpd')) {
            console.log(`[📡] ${pageName.toUpperCase()} MEDIA RES: ${status >= 400 ? '❌' : '✅'} ${status} -> ${url.substring(0, 150)}`);
        }
    });
}
...
async function setupNetworkAdBlocker(page) {
    ...
    try {
        ...
        page.on('request', (request) => {
            const url  = request.url().toLowerCase(), type = request.resourceType();
            if (request.isNavigationRequest() && request.frame() === page.mainFrame()) {
                if (['fanduel','bet','casino','adrevenue'].some(k => url.includes(k))) {
...
async function applyPreloadFirewall(page) {
    ...
    try {
        await page.evaluateOnNewDocument(() => {
            ...
            Object.defineProperty(window, 'onbeforeunload', { configurable: true, get: () => null, set: () => null });
...
async function initializeVideo(page, startMuted, isActivePage) {
    try {
        console.log('[*] Enforcing Video Playback...');
        ...
        while (!playing && tries < 15) {
            for (const frame of page.frames()) {
                try {
                    ...
                    if (btn) { await frame.evaluate(el => el.click(), btn); playing = true; break; }
            ...
            try {
                ...
                const result = await Promise.race([
                    frame.evaluate(() => {
                        ...
                        if (bodyText.includes('stream error') || bodyText.includes('error: forbidden')) {
...
async function performHotSwap(reason) {
    ...
    } catch (err) { console.log(`[⚠️] Swap error: ${err.message}`); }
}

async function startWatchdog() {
    let lastActiveTime = -1, lastDecodedFrames = -1, frozenCheckTimestamp = Date.now();
    let watchdogTicks = 0, isWarmupPhase = true, currentStreamStartTime = Date.now();
    ...
    while (true) {
        ...
        if (activeStatus.status === 'HEALTHY' && !isWarmupPhase) {
            if ((Date.now() - currentStreamStartTime) > FORCE_REFRESH_MS) {
        ...
        if (activeStatus.status === 'HEALTHY') {
            ...
            if ((activeStatus.currentTime === lastActiveTime) || (activeStatus.decodedFrames === lastDecodedFrames && activeStatus.decodedFrames > 0)) {
        ...
        if (['FROZEN', 'CRITICAL_ERROR', 'DEAD', 'FORCE_REFRESH'].includes(activeStatus.status)) {
            ...
            lastActiveTime = -1; frozenCheckTimestamp = Date.now(); currentStreamStartTime = Date.now();
        }
...
async function main() {
    ...
    try { await activePage.goto(activeUrl, { waitUntil: 'domcontentloaded', timeout: 45000 }); } catch (e) {}

    try { await activePage.mouse.click(10, 10); } catch(e) {}

    await initializeVideo(activePage, false, true);

    console.log(`\n[🟢] ALL SET! STREAM IS LIVE ON FULLSCREEN TABS\n`);
    await startWatchdog();
}
