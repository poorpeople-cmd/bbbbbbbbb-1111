...
process.on('uncaughtException', (err) => console.log(`[⚠️] IGNORED UNCAUGHT EXCEPTION: ${err.message}`));
process.on('unhandledRejection', (reason) => console.log(`[⚠️] IGNORED UNHANDLED REJECTION`));
...
let urlList = rawUrls !== '' ? rawUrls.split(',').map(u => u.trim()) : ['https://dadocric.st/player.php?id=starsp3&v=m'];
...
// =========================================================================================
// 🕵️ NETWORK SNIFFER: Extracts the hidden M3U8 Master Playlist
// =========================================================================================
async function extractM3U8(targetUrl) {
    console.log(`\n[🔍] SNIFFER: Booting Headless Chrome to intercept stream link from: ${targetUrl}`);
    
    let browserArgs = [
        '--no-sandbox', 
        '--disable-setuid-sandbox',
        '--disable-web-security',
        `--disable-extensions-except=${process.cwd()}/ublock-lite`,
        `--load-extension=${process.cwd()}/ublock-lite`
    ];

    // Proxy specifically applied to puppeteer
    if (PROXY_ENGINE.includes('Cloudflare')) {
        browserArgs.push('--proxy-server=socks5://127.0.0.1:40000');
    }

    // Now we can use TRUE Headless mode since we don't need screen rendering
    ...
    // 🔥 THIS IS THE CORE ENGINE: Intercepting network traffic
    page.on('request', req => {
        const url = req.url();
        // Look for the main streaming playlist
        if (url.includes('.m3u8')) {
            const headers = req.headers();
            if (!extractedData) {
                // Save the link and the required bypass headers
                extractedData = { 
                    url: url, 
                    referer: headers['referer'] || targetUrl, 
                    userAgent: headers['user-agent'] 
                };
                console.log(`[🎯] BINGO! Intercepted M3U8 Stream Link.`);
            }
        }
    });
    ...
    try {
        ...
        // Wait up to 16 seconds (warmup threshold) to capture the stream packet
        for(let i = 0; i < 16; i++) {
            if (extractedData) break;
            await new Promise(r => setTimeout(r, 1000));
            // Trigger play buttons if the player requires interaction to release the m3u8
            await page.evaluate(() => {
                document.querySelectorAll('button, .play, .jw-icon-display, video').forEach(b => { try{b.click()}catch(e){} });
            }).catch(()=>{});
        }
    } catch(e) {
        console.log(`[⚠️] Sniffer navigation error: ${e.message}`);
    }

    await browser.close();
    console.log(`[🧹] Sniffer Browser Closed. RAM completely freed.`);
    return extractedData;
}

// =========================================================================================
// 🚀 FFMPEG DIRECT PASSTHROUGH (0% Video Rendering)
// =========================================================================================
async function startFFmpegStream(streamData) {
    return new Promise((resolve) => {
        ...
        ffmpegProcess.stderr.on('data', (data) => {
            ...
            // Optional: Print errors if you want to track connection drops
            if (msg.includes('Error') || msg.includes('Server error')) {
                console.log(`[FFmpeg Alert]: ${msg.trim()}`);
            }
        });

        ffmpegProcess.on('close', (code) => {
            console.log(`\n[🛑] Stream Dropped (FFmpeg exited with code ${code}).`);
            resolve(code); // Resolves the promise to restart the loop
        });
    });
}

// =========================================================================================
// 🔄 THE SELF-HEALING MAIN LOOP
// =========================================================================================
async function mainLoop() {
    while (true) {
        let activeUrlStr = urlList[currentUrlIndex];
        
        // 1. Send the sniffer to extract the secret stream link
        let streamData = await extractM3U8(activeUrlStr);

        if (!streamData) {
            console.log(`[❌] Failed to extract M3U8 from Server [${currentUrlIndex}]. Changing server...`);
            currentUrlIndex = (currentUrlIndex + 1) % urlList.length;
            await new Promise(r => setTimeout(r, 5000));
            continue;
        }

        // 2. Pipe the stream directly to OK.RU
        // Yeh process tab tak lock rahega jab tak stream piche se hang/drop nahi hoti
        await startFFmpegStream(streamData);
        
        // 3. Auto-Recovery (Hunting Mode)
        console.log(`[🔄] Initiating Recovery Mode: Re-sniffing stream in 3 seconds...`);
        await new Promise(r => setTimeout(r, 3000));
        
        // Agar drop hua hai, agle server par move kar jao
        currentUrlIndex = (currentUrlIndex + 1) % urlList.length;
    }
}
